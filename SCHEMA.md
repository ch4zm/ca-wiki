...
**Pages capture current truth. Nowhere else. The page body is not a log.**

The wiki records the state of things, not the trail by which we arrived at that state.
Chronology narration on pages is actively harmful: it swamps the factual statements the
wiki exists to hold, and it confuses future readers (including future you and future
LLMs) about what is currently true.

- **Apply corrections silently, in the page body.** When a claim is corrected — by the
  user, a new source, or your own re-reading — the ONLY change to the page body is:
  delete the wrong text, write the right text. Bump `updated:` in frontmatter. Nothing
  else on the page changes. There is nowhere on the page for "previously this said X" —
  not a header, not a paragraph, not a parenthetical, not a footnote, not an HTML
  comment, not a `note:` field. If you're hunting for a spot, there is no spot.
- **No provenance-of-ideas theater.** A claim carries a citation to the source that
  supports it. It does NOT carry cross-attribution for how the current phrasing emerged,
  who first noticed a nuance, or how many revisions preceded it.
- **No date-stamped update sections.** Never append `## <date> update:` blocks or
  `**Update:**` paragraphs. Edit in place; bump `updated:`. The commit (or `log.md`
  entry) records what changed and why.
- **No cross-page correction manifests.** When one correction sweeps multiple pages, each
  page just carries its new correct text. Do NOT list "pages affected by this correction:
  [[a]], [[b]], [[c]]" anywhere. The commit diff shows the sweep.

**Banned patterns in page bodies** — if any of these phrases appears in text you are
writing to a page, delete it before writing:

- "Previously this page said…" / "This was earlier drafted as…"
- "An earlier reading / interpretation was…"
- "Corrected on <date>" / "Updated to reflect…" / "As originally understood…"
- "It was previously stated / believed / understood that…"
- "We now believe / have discovered / have determined…"
- "This claim has been withdrawn / revised / superseded"
- "Note: this used to say…"
- `## Correction`, `## Update <date>`, `**Correction:**`, `**Update:**` headers
- Any bullet whose only content is a refutation of a claim not otherwise present

Skills MUST follow this discipline on every write to a page body. This is the rule the
anti-pedantry common-mistake bullets in `wiki-ingest` and `wiki-update` point back to.
...
- page discipline: pages capture current truth, not revision history (see Page Discipline). Apply corrections silently; no "previously this said X" narration, no `## <date> update:` sections, no cross-attribution for how phrasing emerged. Bump `updated` and let the operation log carry the "why".
