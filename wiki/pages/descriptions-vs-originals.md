...
an attached tape L, a self-reproducer would have to contain its own plan. That is
impossible, because a full description of an automaton takes more cells than the automaton
itself.[^2]

**The way out.** A fixed, finite copier B can copy a description L of *any* size. Copying
...
product F yields viable offspring that produce F′ instead. This is a change of hereditary
strain.[^8]
...
[^2]: [[tsra-part2-ch1](pages/tsra-part2-ch1.md)] p.118 [synthesis] — the a priori argument that the constructor is more complex than what it constructs; a self-reproducer would have to contain its own L, which is larger than the automaton
[^3]: [[tsra-part2-ch1](pages/tsra-part2-ch1.md)] pp.119, 121 [synthesis] — B explores L and makes an exact copy; although B is fixed and finite it copies an L of any size, which transcends the rule that the primary must be superior to the secondary
...
[^8]: [[tsra-part2-ch1](pages/tsra-part2-ch1.md)] pp.130-131 [synthesis] — a mutation in the F part of the description yields a viable, self-reproducing successor producing F′: a change of hereditary strain
